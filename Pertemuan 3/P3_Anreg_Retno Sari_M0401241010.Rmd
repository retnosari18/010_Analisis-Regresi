...
```{r}
library(readxl)
dt <- read_xlsx("C:\\Users\\retno\\Documents\\MATERI KULIAH\\Semester 4\\ANALISIS REGRESI\\DATA PROJEK\\Data Harapan Hidup Fix.xlsx")
dt
```
...
harapan dari peubah respons Y (Harapan Hidup)

$$E[\hat{Y}] = \hat{Y} = 68.92143 + 0.0001876207 X$$
...
bernilai 3945.7 dan JKG model bernilai 2841.5
...
19.6 . Hubungan antara keduanya dapat dituliskan sebagai berikut.
...
$S_e = \sqrt{19.59636} \approx 4.426777$
...
parameter β0 dan β1 sebesar 80.71073 dan -3.967645. Kedua nilai dugaan
...
kepercayaan untuk parameter $β_0$ adalah [67.87313 , 69.96973] dan untuk
parameter $β_1$ adalah [0.0001614874 , 0.000213754]
...
```{r}
amatan.diduga <- data.frame(x=30000)
predict(model, amatan.diduga, interval = "confidence") #rataan
```

Berdasarkan output sintaks di atas, diketahui bahwa dugaan rataan nilai
$Y$ ketika nilai x=30000 adalah 74.55005. Selain itu, diindikasikan juga
bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai dugaan rataan
$Y$ ketika nilai x=30000 berada dalam selang 73.82803 hingga 75.27207 .
...
Misalkan kita ingin menduga nilai individu amatan ketika nilai x=30000.
Kita dapat memanfaatkan fungsi `predict` untuk mendapatkan nilai dugaan
dan selang kepercayaannya sebagai berikut.

```{r}
predict(model, amatan.diduga, interval = "prediction") #individu
```

Berdasarkan output sintaks di atas, diketahui bahwa dugaan nilai
individu Y ketika nilai x=10 adalah 74.55005 . Selain itu, diindikasikan
juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai amatan
individu Y ketika nilai x=30000 berada dalam selang 65.77096 hingga
83.32914 .
